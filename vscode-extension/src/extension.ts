...
export function activate(context: vscode.ExtensionContext) {
  ...
  const watcher = vscode.workspace.createFileSystemWatcher(
    new vscode.RelativePattern(eventsDir, '*.json')
  );
    ...
    try {
      ...
      if (event.type === 'blocked') {
        ...
        vscode.window.showWarningMessage(
          `🚨 Counter Agent blocked: ${event.analysis?.reason || 'Unknown reason'}`,
          'Show Details'
        ).then(selection => {
...
function showMonitor(context: vscode.ExtensionContext) {
  if (panel) {
